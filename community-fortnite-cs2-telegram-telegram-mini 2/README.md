...
Локальный MVP-прототип community-платформы ECS для Fortnite и CS2 турниров с фокусом на Telegram Mini App UX и esports dashboard-интерфейс.
...
- Регистрация на турнир на мок-данных.
- Выдача room code / island code только после регистрации.
- Профиль игрока, история участия, очки и место в рейтинге.
- Поиск игроков по нику, Telegram, Epic и Steam.
- Публичная карточка игрока с локальной статистикой лиги.
...
- Admin-панель для добавления турниров.
- Admin-обновление Fortnite / CS2 code.
- Admin-редактирование CS2 2v2 матчей: команды, счёт и статус.
- Заготовка под Telegram Web Apps SDK: если приложение откроется внутри Telegram, профиль подтянется из `window.Telegram.WebApp.initDataUnsafe.user`.
...
3. Подключить Telegram Bot auth validation на backend.
